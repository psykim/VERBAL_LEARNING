...
const CERADMemoryTest = () => {
  ...
  const calculateResults = () => {
    ...
    const recognitionScore = recognitionTotal - 10; // 새로운 재인 점수 계산
    ...
    setResults({
      ...
      scores: {
        ...
        recognition: {
          ...
          score: recognitionScore // 재인 점수 추가
        }
      },
      // 파생 지표
      derived: {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl text-center">CERAD NP WLMT/WLRT/WLRcT Analyzer</CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-4 gap-3">
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                            onClick={() => showScoreDescription('trial1')}
                          >
                            시행 1 (/10) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-xl font-bold">{results.scores.trial1}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          >
                            시행 2 (/10) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-xl font-bold">{results.scores.trial2}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          >
                            시행 3 (/10) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-xl font-bold">{results.scores.trial3}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          >
                            총 학습량 (/30) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-xl font-bold">{results.scores.totalLearning}</p>
                        </div>
                      </div>
                      
                      {/* 두 번째 줄 - 지연 회상과 재인 점수들 */}
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div>
                          ...
                          >
                            지연 회상 (/10) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-xl font-bold">{results.scores.delayedRecall}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          >
                            재인 (/10) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-xl font-bold">{results.scores.recognition.score}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          >
                            정확 재인 (/10) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-xl font-bold">{results.scores.recognition.hits}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          >
                            정확 기각 (/10) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-xl font-bold">{results.scores.recognition.correctRejections}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-4 gap-3">
                        <div>
                          ...
                          <p className="text-lg font-semibold">{results.derived.learningSlope}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.derived.learningRate.toFixed(1)}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.derived.learningEfficiency.toFixed(1)}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.derived.savingsScore.toFixed(1)}</p>
                          <Badge variant={results.derived.savingsScore < 50 ? "destructive" : "default"} className="text-xs">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-semibold">{results.derived.forgettingRate.toFixed(1)}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.derived.retentionIndex.toFixed(1)}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.derived.consistencyPercent.toFixed(1)}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.derived.retrievalEfficiency.toFixed(1)}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.derived.recognitionEfficiency}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-4 gap-3">
                        <div>
                          ...
                          <p className="text-lg font-semibold">{results.derived.dPrime.toFixed(2)}</p>
                          <Badge variant={results.derived.dPrime < 1.5 ? "destructive" : "default"} className="text-xs">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-semibold">{results.derived.beta.toFixed(2)}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-600">오경보</p>
                          <p className="text-lg font-semibold">{results.scores.recognition.falseAlarms}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-600">누락</p>
                          <p className="text-lg font-semibold">{results.scores.recognition.misses}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-4 gap-3">
                        <div>
                          ...
                          <p className="text-lg font-semibold">{results.errors.intrusions}</p>
                          <Badge variant={results.errors.intrusions > 5 ? "destructive" : "default"} className="text-xs">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-semibold">{results.errors.repetitions}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-600">침입률 (%)</p>
                          <p className="text-lg font-semibold">{results.errors.intrusionRate.toFixed(1)}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-600">반복률 (%)</p>
                          <p className="text-lg font-semibold">{results.errors.repetitionRate.toFixed(1)}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-4 gap-3">
                        <div>
                          ...
                          >
                            초반 회상 (%) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-lg font-semibold">{results.serialPosition.primacyEffect.toFixed(1)}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-600">중반 회상 (%)</p>
                          <p className="text-lg font-semibold">{results.serialPosition.middleRecall.toFixed(1)}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          >
                            후반 회상 (%) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-lg font-semibold">{results.serialPosition.recencyEffect.toFixed(1)}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        <div>
                          <h5 
                            className="font-semibold mb-2 cursor-pointer hover:underline flex items-center gap-1 text-sm"
                    ...
                    <CardContent>
                      <div className="grid grid-cols-4 gap-3">
                        <div>
                          ...
                          >
                            첫 시행 기억 (/10) <Info className="h-3 w-3" />
                          </p>
                          <p className="text-lg font-semibold">{results.additional.firstTrialMemory}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.additional.estimatedLTS}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-600">학습 패턴</p>
                          <p className="text-lg font-semibold">{results.additional.learningPattern}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.additional.contrastScaledScore}</p>
                        </div>
                        <div>
                          <p 
                            className="text-xs text-gray-600 cursor-pointer hover:underline flex items-center gap-1"
                          ...
                          <p className="text-lg font-semibold">{results.additional.cueingEffect}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
